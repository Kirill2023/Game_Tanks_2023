...
public:
    static unsigned char* Load_image(const std::string filename, int* width, int* height, unsigned char Alpha = 127)
    {
        ...
        *height = *(int*)&(header[0x16]); // Высота
        ...
        for (int j = 0, i = 0; i < imageSize; j += 4, i += 3)
        {
            data2[j + 0] = data[i + 2];
            data2[j + 1] = data[i + 1];
            data2[j + 2] = data[i + 0];
    ...
    TEX(std::string name)
    {
        ...
        int width, height;
        unsigned char* image = Load_image(name, &width, &height, 180);
...
public:
    ...
    static void Draw(VAO& vao, const spTEX sptex, const glm::mat4& model, const glm::mat4& view, const glm::mat4& projection);
